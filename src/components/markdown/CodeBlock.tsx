...
const CodeBlock: Component<{ node: MdNode }> = (props) => {
  ...
  return (
    <Show
      when={html()}
      fallback={
        <pre class="overflow-x-auto rounded-lg bg-gray-100 p-4 text-[0.85em] text-gray-800 dark:bg-gray-900 dark:text-gray-100">
          <code>{props.node.literal ?? ""}</code>
        </pre>
      }
    >
      <div
        class="code-block overflow-x-auto rounded-lg text-[0.85em] [&>pre]:p-4"
        innerHTML={html()!}
      />
    </Show>
  );
